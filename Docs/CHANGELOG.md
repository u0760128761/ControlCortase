...
- **Очистка кода**: Удалены закомментированные куски устаревшего кода(мусор) из `motor_server.py`. Логика матрицы движений (`process_movement_cmd`) возвращена к первозданному «прямому» виду, так как аппаратная путаница (перехлест пинов) решается штатным назначением ролей в веб-интерфейсе и приложении.
...
### Исправлено
- **Зависание UI конфигурации в Android**: Исправлен баг в мобильном приложении (`ControlActivity.kt`), из-за которого экран управления не обновлял распиновку после возврата из экрана настроек. Логика запроса `GET_CONFIG` перенесена из `onCreate` в `onResume`.
